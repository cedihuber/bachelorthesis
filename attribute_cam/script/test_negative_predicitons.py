...
def command_line_options():
    ...
    parser.add_argument(
        '-o',
        '--output-directory',
        default="../../../../local/scratch/chuber/result/check_negativ_attribute",
...
# Generate masks
def generate_masks(patch_size, image_size=(224, 224)):
    
    masks =  torch.ones((1, 1, image_size[0], image_size[1]), dtype=torch.float32, device=device)
    ...
    masks[0, 0, y:y+patch_size, x:x+patch_size] = patch
            
    return masks

def save_masks_as_images(masks, output_dir,image_index):
    ...
    for i, mask in enumerate(masks):
        ...
        img.save(os.path.join(output_dir, f"mask_{image_index}.png"))


def apply_and_save_masks(image, masks, img_path):

    if image.dim() == 4:
        image = image.squeeze(0)
    original = image.clone()
    # Save as PNG
    #torchvision.io.image.write_png((original.cpu() * 255).byte(), img_path)
    perturbed_images = []
    ...
    #print(img_name)
    # Generate filenames
    perturbed_filenames = [f'perturbed_image_{img_name}']
...
def main():
    ...
    # for attribute_idx in range(0,40):        
    #    os.makedirs(f'{args.output_directory}/{attribute_cam.dataset.ATTRIBUTES[attribute_idx]}', exist_ok=True)
    os.makedirs(f"{args.output_directory}/scores", exist_ok=True)
    
    # get some directories
    file_lists = [os.path.join("../../../../local/scratch/chuber/result/corrRise_masks_blurry_1batchs_60size_500masks/img_names.txt")]
    ...
    prediction_file = attribute_cam.prediction_file("./result", "test", "balanced")
    prediction = attribute_cam.read_list(prediction_file, ",", 0)
    
    startTime = datetime.now()
    
    masks = generate_masks(50)
    masks = masks.to(device)
    
    no_masks = generate_masks(0)
    no_masks = no_masks.to(device)
    ...
    for filter_type in ["pr=-1"]: #"pr=1",
    
        # create dataset
        celeba_dataset = attribute_cam.CelebA_perturb(
            file_lists,
        ...
        #for attribute in tqdm(celeba_dataset.attributes):
        attribute = "Smiling"
        overlays = torch.zeros([3,224,224])
        activations = np.zeros((224,224), dtype=np.float64)
        print(attribute)
        average = 0
        num_positiv = 0
        num_negativ = 0
        for image_index in tqdm(celeba_dataset):
            # apply filter function to get only the images that we want
            # print(f"not filtered {attribute} {image_index}")
            if filter(image_index,attribute):
                # load images
                # print(image_index,end="\n")
                img_path = f"{args.source_directory}/{image_index}.png"
                image, orig_image = load_img(img_path)
                perturbed_images = apply_and_save_masks(image, masks, img_path)
                original_images = apply_and_save_masks(image, no_masks, img_path)
                save_masks_as_images(image,f'{args.output_directory}/originals', image_index)
                #print(perturbed_images[0].shape)
                save_masks_as_images(perturbed_images[0],f'{args.output_directory}/masks_images', image_index)
                scores_of_originals = affact.predict_logit(original_images)
                scores_of_images = affact.predict_logit(perturbed_images)
                
                scores_of_images = scores_of_images.squeeze().tolist()
                scores_of_originals = scores_of_originals.squeeze().tolist()
                
                average += scores_of_originals[31] - scores_of_images[31]
                if(scores_of_originals[31] - scores_of_images[31] > 0):
                    num_positiv += 1
                else:
                    num_negativ += 1
                with open(f"{args.output_directory}/scores/perturbed_scores.csv", "a", newline="") as f:
                    writer = csv.writer(f)
                             # shape [40]
                    row = [image_index] + [scores_of_images[31]]                         # prepend image_index
                    writer.writerow(row)
                    
                with open(f"{args.output_directory}/scores/original_scores.csv", "a", newline="") as f:
                    writer = csv.writer(f)
                    
                    row = [image_index] + [scores_of_originals[31]]                         # prepend image_index
                    writer.writerow(row)
                
                with open(f"{args.output_directory}/scores/difference_scores.csv", "a", newline="") as f:
                    writer = csv.writer(f)
                    row = [image_index] + [scores_of_originals[31]-scores_of_images[31]]                      # prepend image_index
                    writer.writerow(row)
                    
                    
        # print(f'activation{activation.max()}')
        print(f'average {average} positiv{num_positiv} negativ{num_negativ}')
        #print(f"Averaging RISE of type for {filter_type} filter and {len(dataset.attributes)} attributes")
    
    # compute average images
    #attribute_cam.average_cam(dataset, filter)
    # attribute_cam.average_perturb(dataset, filter)
