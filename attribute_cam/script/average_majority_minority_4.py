...
def command_line_options():
  ...
  parser.add_argument(
      '-o', '--output-directory',
      default="../../../../local/scratch/chuber/corrRiseStyles/balanced/corrRise_masks_black_3000_masks_30_patch", #corrRiseResult/unbalanced/
...
def main():
  ...
  file_lists = [os.path.join(args.output_directory,"img_names.txt")]
  cam_directory = args.output_directory
  # read ground truth, predictions and majority 
  ...
  training_ground_truth = os.path.join(args.protocol_directory, "training.txt")
  training = attribute_cam.read_list(training_ground_truth, " ", 2)
  
  values = list(training.values())
  print(len(values))
  majority_dict = {key: 0 for key in values[0]}
  for key_p, value_p in training.items():
  ...
  majority_dict = {key: 1 if value >= 0 else -1 for key, value in majority_dict.items()}
  with open(file_path, 'w') as f:
    json.dump(majority_dict, f)
  ...
  for filter_type in args.filters:
    ...
    # create dataset
    dataset = attribute_cam.CelebA_perturb(
        file_lists,
    ...
    filter = attribute_cam.Filter1(ground_truth, prediction, majority_dict, filter_type)
