...
device = torch.device("cuda:4" if torch.cuda.is_available() else "cpu") # mit : cuda: 0 kann ich angeben auf welcher gpu nummer, gpustat um gpu usage zu schauen
...
def command_line_options():
    ...
    parser.add_argument(
        '-o',
        '--output-directory',
        default="../../../../local/scratch/chuber/Finalresults/unbalanced/corrRise_masks_black_white_3000_masks_30_patch",
...
def pearson_correlation_multi(x, y, original_score): # x shape (500,40) y shape (500,50176)
...
def process_saliency(attribute_idx, saliency_maps, orig_image, img_name_no_ext, attribute_name, celebA_dataset, args):
    ...
    # celebA_dataset.save_perturb(positive_saliency,orig_image, f'{args.output_directory}/{attribute_name}/{img_name_no_ext}.png')
