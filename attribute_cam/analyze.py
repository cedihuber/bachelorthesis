...
def run(args):
    ...
    dir_imgs = '/local/scratch/datasets/CelebA/aligned_224x224'
...
#    folder_imgs = input_dir.split('\\')[-1].split('.txt')[0]
    folder_imgs = os.path.splitext(os.path.basename(input_dir))[0]
    ...
    with open(path_csv, 'w', newline='') as f3:
        ...
        for path in tqdm(img_paths[:args.nr_imgs]): # iterate over all the images

            # extract img number
#            img_number = path.split("\\")[-1].split('.')[0]
            img_number = os.path.splitext(os.path.basename(path))[0]
            ...
            ground_truth_values = ground_truth[ground_truth['image'] == f'{img_number}.png']
            # convert to list
            ground_truth_values = ground_truth_values.values.tolist().pop()
            # remove {img_number}.png
...
def main():
    ...
    parser.add_argument(
        '--input_dir',
        type=str,
        default="./files/aligned_224x224_test_filtered_0.1.txt",
        help="Path to folder containing the input images"
    )
    parser.add_argument(
        '--path_gt',
        type=str,
        default="./files/ground_truth_celeba.txt",
...
#    )
    ...
    args.use_cuda = args.gpu is not None
    if args.use_cuda:
